...
st.title("📄 RAG_ChatBot ST & LC")

# Crear estructura de directorios en el directorio padre
parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
directories = ["data", "chroma_langchain_db"]
for directory in directories:
    dir_path = os.path.join(parent_dir, directory)
    os.makedirs(dir_path, exist_ok=True)
    print(f"✅ Directorio {directory} creado o verificado en {parent_dir}")

# Inicializar el historial del chat
if "messages" not in st.session_state:
    st.session_state.messages = []

# Variable para controlar si el documento ya fue procesado
...
vector_store = Chroma(
    collection_name="ElderLink_collection",
    embedding_function=embeddings,
    persist_directory=os.path.join(parent_dir, "chroma_langchain_db"),  
)
...
if uploaded_file and not st.session_state.document_processed:
    try:
        with st.spinner("⏳ Procesando documento..."):
            data_path = os.path.join(parent_dir, "data", uploaded_file.name)
...
# Guardar el gráfico dentro de la carpeta data
img_bytes = graph.get_graph().draw_mermaid_png()
graph_path = os.path.join(parent_dir, "data", "langgraph_visualization.png")
with open(graph_path, "wb") as f:
    f.write(img_bytes)
print(f"El gráfico ha sido guardado como '{graph_path}'")